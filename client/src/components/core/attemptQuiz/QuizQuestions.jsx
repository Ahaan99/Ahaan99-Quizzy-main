    ...
    const handleAnswerChange = useCallback((questionId, selectedOption) => {
        setUserAnswers(prevAnswers => {
            ...
            if (existingAnswerIndex >= 0) {
                // Fixed: avoid direct mutation, create new array with updated item
                return prevAnswers.map((ans, i) =>
                    i === existingAnswerIndex ? { ...ans, selectedOption } : ans
                );
            }
    ...
    const submitQuiz = async () => {
        try {
            ...
            navigate('/quiz-results', {
                state: { score: response.data.score, total: response.data.total ?? quizQuestions?.length }
            });
        } catch (error) {
